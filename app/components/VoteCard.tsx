...
export default function VoteCard({
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          ...
          schema: "public",
          table: "shares",
          filter: `representative_id=eq.${id}`,
        },
        () =>
  ...
  const total =
    approve_count + disapprove_count;

  const approvalPct =
    total > 0
      ? Math.round(
          (approve_count / total) * 100
        )
      : 0;
  ...
  return (
    <div className="border border-white/10 bg-white/[0.02] p-6 relative group hover:border-yellow-400 transition flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        <span
          className={`text-xs font-black px-3 py-1 uppercase tracking-wider ${tagColor}`}
        >
          {category}
        </span>
        ...
        {rank && (
          <span className="border border-yellow-400/40 text-yellow-400 text-xs font-black px-3 py-1 uppercase tracking-wider">
            #{rank}
          </span>
        )}
      ...
      <div className="mb-2">
        ...
        <div className="flex justify-between mt-1.5 text-xs">
          <span className="text-yellow-400 font-bold">
            {approve_count.toLocaleString()} ·{" "}
            {approvalPct}%
          </span>

          <span className="text-red-400 font-bold">
            {disapprovalPct}% ·{" "}
            {disapprove_count.toLocaleString()}
          </span>
        </div>
      </div>
